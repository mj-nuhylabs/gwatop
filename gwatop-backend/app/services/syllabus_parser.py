...
(R4) **과제는 "마감/제출/Due/까지" 가 명시된 날짜만 due_date 로 쓴다.**
     - "과제 1 출제" 만 있고 마감이 같이 적혀있지 않으면 assignments 항목을 만들지 말고
       해당 weeks.notes 에 출제 정보를 그대로 남긴다.
     - 같은 과제의 "출제일" 정보를 알면 description 에 "출제: YYYY-MM-DD" 형태로 보존한다.
     - "과제 1 마감 23:59" → assignments: title="과제 1", due_date="YYYY-MM-DD", description="마감 23:59"
...
FEWSHOT_ASSISTANT = json.dumps(
    {
        ...
        "assignments": [
            {"title": "과제 1", "due_date": "2026-06-19", "description": "마감 23:59 / 출제: 2026-06-12"},
            {"title": "과제 2", "due_date": "2026-06-30", "description": "마감 23:59 / 출제: 2026-06-26"},
            {"title": "과제 3", "due_date": "2026-07-10", "description": "출제: 2026-07-08"},
        ],
        "confidence": 0.95,
        "warnings": [],
    },
...
def _already_has_assignment(syllabus: ParsedSyllabus, title_hint: str, d: date) -> bool:
    ...
    for a in syllabus.assignments:
        if a.due_date == d and re.sub(r"\s+", "", a.title).lower() == title_norm:
            return True
        if a.due_date == d:
            return True
    return False
...
def _assignment_title_from_segment(seg: str) -> str:
...
def _recover_missing_events(
    ...
    for w in syllabus.weeks:
        ...
        for seg in _split_segments(w.notes):
            ...
            is_assignment_seg = (
                any(kw in seg or kw.lower() in seg_lower for kw in _ASSIGN_KEYWORDS)
                and has_due
            )
            ...
            if is_assignment_seg:
                title = _assignment_title_from_segment(seg)
                if _already_has_assignment(syllabus, title, d):
                    continue
                try:
            ...
            # 시험·과제 키워드는 없지만 날짜 + ★/마감 같은 신호가 있는 모호한 항목 → warning
            if "★" in seg or "☆" in seg:
                leftovers.append(seg[:160])
