...
(R4) **assignments 에는 실제 '제출물'의 마감일만 넣는다.** (시험/퀴즈는 R3 대로 exams)
     ★ **title 은 강의계획서에 적힌 과제의 실제 이름을 그대로 쓰고 끝에 '마감' 을 붙인다.** ★
       예) "Team Project Presentation 마감", "Case Analysis 마감", "Self Reflection Essay 마감",
           "Personal Business Plan 마감", "보고서 2 마감".
       한국어/영어 원문 이름을 최대한 보존한다 (영어면 영어 그대로).
     - ⚠️ 자료에 이름이 있으면 **절대 "과제 1/2/3" 로 일반화하지 마라. 없는 번호를 임의로 붙이지 마라.**
       실제 이름을 도저히 못 찾을 때만 "과제 마감" 을 쓴다 (자료에 '과제 3' 이라고 명시된 경우만 "과제 3 마감").
     - ⚠️ **시험(Exam/시험/중간/기말/Quiz)은 assignments 에 넣지 마라 — 그건 exams 다.**
       'Exam 1 on 7/9', '중간고사 7/16' 같은 걸 '과제 1 마감' 으로 만들지 마라.
     - 출제일이 함께 명시돼 있으면 description 에 "출제 M/D" 형식으로 보존한다.
     - 출제일만 있고 마감일이 명시 안 됐으면 assignments 에 넣지 마라 (weeks.notes 에만 보존).
     - **'제출/Due/까지/submit/turn in' 도 '마감'으로 간주**, 같은 키워드군이다.
...
(R3) **퀴즈/쪽지시험/미니테스트는 exams 에**, **과제(제출물) 마감만 assignments 에**.
     title 예(시험): "퀴즈 1", "중간고사", "Exam 1".

(R4) **assignments 에는 실제 제출물의 마감일만 넣는다. 시험은 넣지 마라(exams).**
     - ★ **title 은 자료에 적힌 과제의 실제 이름 + '마감'** (예: "Case Analysis 마감",
       "Team Project Presentation 마감", "Essay 마감"). 영어면 영어 그대로 보존.
     - ⚠️ 이름이 있으면 "과제 1/2/3" 로 일반화 금지, 임의 번호 금지. 이름을 못 찾을 때만 "과제 마감".
     - ⚠️ 'Exam 1 on 7/9' 처럼 시험을 '과제 N 마감' 으로 만들지 마라.
     - 출제일이 함께 있으면 description 에 "출제 M/D" 로 보존. 마감일 없으면 넣지 마라.
     - '제출/Due/까지/submit' 은 '마감'으로 간주.
