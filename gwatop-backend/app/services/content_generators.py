...
8. **수학 수식·기호는 반드시 LaTeX 로 작성**. 인라인은 `$...$`, 별도 줄은 `$$...$$`.
   - 예: `$\\int_a^b f(x)\\,dx$`, `$\\sqrt{1+(f'(x))^2}$`, `$$S = 2\\pi \\int_a^b f(x)\\,dx$$`
   - 적분/시그마/분수/제곱근/지수/하첨자 모두 LaTeX 명령으로. 절대 `∫[a,b]`, `√(...)`, `x^2` 같은
     평문으로 쓰지 마라 (앱이 보기 흉하게 표시함).
   - JSON 문자열 안이라 백슬래시는 `\\\\` 로 이스케이프 필요할 때 적절히.
